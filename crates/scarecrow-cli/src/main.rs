...
use scarecrow_cli::cli::{Cli, Commands, expand_path};
use scarecrow_cli::downloader::{download, generate_json};
use scarecrow_cli::uploader::{append, list, login, renew, show, upload_by_command, upload_by_config};

use clap::Parser;

use scarecrow_cli::server::errors::AppResult;
...
async fn main() -> AppResult<()> {
    ...
    match cli.command {
        ...
        Commands::Server { bind, port, auth } => {
            scarecrow_cli::run((&bind, port), auth, console_reload_handle).await?
        }
        Commands::List {
            is_pubing,
            pubed,
